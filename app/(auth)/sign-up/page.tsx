...
import { signUpWithEmail } from "@/lib/actions/authaction";
import { registerFaceOnSignUp } from "@/lib/actions/facial-signup";
...
const SignUp = () => {
  ...
  const [signUpStep, setSignUpStep] = useState<'form' | 'facial'>('form');
  ...
  const onSubmitForm = async (data: SignUpFormData & { confirmPassword?: string; termsAccepted?: boolean }) => {
    setFormError('');
    ...
    try {
      ...
      if (result?.success) {
        // Store data for facial registration
        setAccountData(submitData as SignUpFormData);
        setSignUpStep('facial');
        toast.success('Account created! Set up facial recognition (optional)', {
          description: 'You can also skip this and use email/password.',
        });
      } else {
  ...
  // Handle facial registration success
  const handleFacialSuccess = async (faceDescriptor: string) => {
    ...
    try {
      toast.loading('Registering facial profile...');
      
      const result = await registerFaceOnSignUp(
        accountData.email,
        faceDescriptor,
      ...
      if (result.success) {
        toast.dismiss();
        toast.success('Facial profile registered successfully!');
        router.push('/');
      } else {
        toast.dismiss();
        toast.error(result.error || 'Failed to register facial profile');
      }
    } catch (error) {
      toast.dismiss();
      toast.error('Failed to register facial profile');
      console.error(error);
    }
  };
  ...
  // Skip facial registration and go to dashboard
  ...
  // Facial registration step
  if (signUpStep === 'facial' && accountData) {
    return (
      <>
        <div className="mb-6">
          <h1 className="form-title">Register Your Face</h1>
          <p className="text-sm text-gray-600 mt-2">Optional: Set up faster login with facial recognition</p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="pt-4 border-t border-gray-300 space-y-3">
            <p className="text-sm text-gray-600 text-center">
              You can always add facial recognition later in your account settings.
            </p>

            <Button
              onClick={handleSkipFacial}
              variant="outline"
              className="w-full"
            >
